...
public class HikariPool extends PoolBase implements HikariPoolMXBean, IBagStateListener {
  private Logger LOGGER=LoggerFactory.getLogger(HikariPool.class);
  private static ClockSource clockSource=ClockSource.INSTANCE;
  private static int POOL_NORMAL=0;
  private static int POOL_SUSPENDED=1;
  private static int POOL_SHUTDOWN=2;
  private volatile int poolState;
  private long ALIVE_BYPASS_WINDOW_MS=Long.getLong("com.zaxxer.hikari.aliveBypassWindowMs",MILLISECONDS.toMillis(500));
  private long HOUSEKEEPING_PERIOD_MS=Long.getLong("com.zaxxer.hikari.housekeeping.periodMs",SECONDS.toMillis(30));
  private PoolEntryCreator POOL_ENTRY_CREATOR=new PoolEntryCreator();
  private AtomicInteger totalConnections;
  private ThreadPoolExecutor addConnectionExecutor;
  private ThreadPoolExecutor closeConnectionExecutor;
 ...
 */
  @Override final void recycle(  final PoolEntry poolEntry){
    ...
    @Override public void run(){
      try {
        ...
        if (clockSource.plusMillis(now,128) < clockSource.plusMillis(previous,HOUSEKEEPING_PERIOD_MS)) {
          LOGGER.warn("{} - Retrograde clock change detected (housekeeper delta={}), soft-evicting connections from pool.",poolName,clockSource.elapsedDisplayString(previous,now));
          previous=now;
          softEvictConnections();
          fillPool();
          return;
...
public static class PoolInitializationException extends RuntimeException {
    private static long serialVersionUID=929872118275916520L;
    /** 
