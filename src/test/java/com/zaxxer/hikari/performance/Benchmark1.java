...
/** 
 * This test requires the Javassist library to be present in the classpath. To reproduce our results, it should be run as follows: JVM parameters: -server -XX:+UseParallelGC -Xss256k -Dthreads=200
...
public class Benchmark1 {
  ...
  public long runAndMeasure(  Measurable[] runners,  CountDownLatch latch,  String timeUnit){
      ...
      try {
        ...
        for (int i=0; i < iter; i++) {
          for (int j=0; j < 100; j++) {
            ...
            for (int k=0; k < 100; k++) {
              resultSet.next();
              localCounter+=resultSet.getInt(1);
            }
            connection.close();
          }
        }
