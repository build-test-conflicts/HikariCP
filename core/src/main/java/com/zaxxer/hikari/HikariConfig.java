...
public final class HikariConfig implements HikariConfigMBean {
  private static long CONNECTION_TIMEOUT=5000L;
  private static long ACQUIRE_RETRY_DELAY=750L;
  private static long IDLE_TIMEOUT=TimeUnit.MINUTES.toMillis(10);
  private static long MAX_LIFETIME=TimeUnit.MINUTES.toMillis(30);
 ...
 */
  ...
  /** 
 * Set a package mapping for "shaded" drivers so that we can find the DataSource in the instrumentation codex even though the package/class name is different. The mapping should be of the form:<p> &lt;original package>:&lt;shaded package> <p> Where the original package name is a widely scoped as possible while still being unique to the driver.  Typically, this is the first two segments of a package name.  For example, take the DataSource <code>org.mariadb.jdbc.MySQLDataSource</code>, that has been shaded to <code>com.other.maria.jdbc.MySQLDataSource</code>.  In this case, the following mapping could be used:<p> org.mariadb:com.other.maria <br>
